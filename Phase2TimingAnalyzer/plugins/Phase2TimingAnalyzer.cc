...
Phase2TimingAnalyzer::Phase2TimingAnalyzer(const edm::ParameterSet& iConfig):
  ...
  vertexCollectionToken_(consumes<std::vector<reco::Vertex>>(edm::InputTag("offlinePrimaryVertices4D"))),
...
*/
  ...
  for (const auto & pv_iter : *vertexCollectionH){ //loops over each PV
    //std::cout << pv_iter.x()  << " " << pv_iter.y() << " " << pv_iter.z() << " " << pv_iter.t() << std::endl;
    pv_t.push_back(pv_iter.t());
...
*/
  ...
  tree_.npv = npv;
  for (int it = 0; it < npv; it++){
    tree_.pv_t.push_back(pv_t[it]);
    tree_.pv_x.push_back(pv_x[it]);
    tree_.pv_y.push_back(pv_y[it]);
    tree_.pv_z.push_back(pv_z[it]);
...
void Phase2TimingAnalyzer::beginJob() {
  ...
  tree->Branch("pv_y", &tree_.pv_y);
  tree->Branch("pv_z", &tree_.pv_z);
